...
namespace TAT {
   ...
   void calculate_svd(
      ...
      if (m > n) {
         auto new_a = content_vector<ScalarType>(n * m);
         auto old_u = content_vector<ScalarType>(n * min);
         auto old_vt = content_vector<ScalarType>(min * m);
         // new_a = a^T
         // u s vt = a
         // vt^T s u^T = a^T
         // old_u = vt^T
   ...
   calculate_svd_kernel<float>(const int& m, const int& n, const int& min, const int& max, const float* a, float* u, float* s, float* vt) {
      ...
      auto work = content_vector<float>(lwork);
   ...
   calculate_svd_kernel<double>(const int& m, const int& n, const int& min, const int& max, const double* a, double* u, double* s, double* vt) {
      ...
      const int lwork = int(float_lwork);
      auto work = content_vector<double>(lwork);
   ...
   inline void calculate_svd_kernel<std::complex<float>>(
         ...
         std::complex<float>* vt) {
      int result;
      auto rwork = content_vector<float>(5 * min);
      const int lwork_query = -1;
      ...
      auto work = content_vector<std::complex<float>>(lwork);
   ...
   inline void calculate_svd_kernel<std::complex<double>>(
         ...
         std::complex<double>* vt) {
      int result;
      auto rwork = content_vector<double>(5 * min);
      const int lwork_query = -1;
      ...
      const int lwork = int(float_lwork.real());
      auto work = content_vector<std::complex<double>>(lwork);
